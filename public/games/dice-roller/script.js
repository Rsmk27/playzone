...
btn.onclick = ()=>{ const n = Math.floor((window.crypto.getRandomValues(new Uint32Array(1))[0] / 4294967296)*6)+1; info.textContent = `You rolled: ${n}`; score.textContent=`History: ${(score.textContent.replace('History: ','')+' '+n).trim()}`; }
